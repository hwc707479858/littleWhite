...
**Given** 用户已导入合法报告文件
**When** 用户触发 OCR
**Then** 系统先进入 OCR 导入文件页面并完成文件选择，触发识别后返回 OCR 文本并记录 OCR 完成状态，且在 OCR 结果页展示原文文本区
**And** OCR 失败时在 1 秒内给出可读失败原因并展示重试按钮（保留在 OCR 导入/结果流程内可重试）。
...
**And** 低置信字段被显式标记，非法字段入库事件为 0（本故事聚焦结构化字段级低置信，OCR 原文级低置信由 Story 1.2 负责）。
